...
router.post('/forgot-password', authLimiter, async (req, res) => {
  ...
  try {
    const encryptedEmail = encryptDeterministic(email);
    let result = await query('SELECT id, email FROM users WHERE email = $1', [encryptedEmail]);
    if (!result.rows.length) {
      // Check legacy email
      result = await query('SELECT id, email FROM users WHERE email = $1', [email]);
      if (!result.rows.length) {
        // Return 200 anyway to prevent email enumeration
        return res.json({ message: 'Jika email terdaftar, OTP telah dikirim.' });
      }
    }
    const user = result.rows[0];
    const otpCode = generateOTP();
    ...
    await query('UPDATE users SET otp_code = $1, otp_expires_at = $2 WHERE id = $3', [otpCode, otpExpires, user.id]);
    const decryptedEmail = user.email === email ? email : decryptDeterministic(user.email);
    await sendOTPEmail(decryptedEmail, otpCode).catch(err => console.error('Failed to send OTP:', err));
    
    return res.json({ temp_token: signTempToken(user.id, 'reset_pass_otp'), message: 'OTP sent to email' });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: 'Terjadi kesalahan server.' });
  }
...
router.post('/verify-reset-otp', async (req, res) => {
  ...
  const decoded = verifyTempToken(temp_token, 'reset_pass_otp');
  if (!decoded) return res.status(401).json({ error: 'Sesi kedaluwarsa atau tidak valid.' });
  
  try {
    const result = await query('SELECT otp_code, otp_expires_at FROM users WHERE id = $1', [decoded.id]);
    const user = result.rows[0];
    if (!user || user.otp_code !== otp_code || new Date() > user.otp_expires_at) {
      return res.status(400).json({ error: 'OTP salah atau sudah kedaluwarsa.' });
    }
    
    await query('UPDATE users SET otp_code = NULL, otp_expires_at = NULL WHERE id = $1', [decoded.id]);
    
    return res.json({ temp_token: signTempToken(decoded.id, 'reset_pass'), message: 'OTP verified' });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: 'Terjadi kesalahan server.' });
  }
...
router.post('/reset-password', async (req, res) => {
  ...
  if (!temp_token || !password || password.length < 8 || !/[A-Z]/.test(password) || !/[0-9]/.test(password)) {
    return res.status(400).json({ error: 'Password tidak valid (minimal 8 karakter, ada huruf kapital dan angka).' });
  }
  
  const decoded = verifyTempToken(temp_token, 'reset_pass');
  if (!decoded) return res.status(401).json({ error: 'Sesi kedaluwarsa atau tidak valid.' });
...
router.post('/forgot-pin', async (req, res) => {
  ...
  if (!temp_token) return res.status(400).json({ error: 'Missing token' });
  ...
  try {
    ...
    await query('UPDATE users SET otp_code = $1, otp_expires_at = $2 WHERE id = $3', [otpCode, otpExpires, user.id]);
    await sendOTPEmail(user.email, otpCode).catch(err => console.error('Failed to send OTP:', err));
    
    return res.json({ temp_token: signTempToken(user.id, 'reset_pin_otp'), message: 'OTP sent to email' });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: 'Terjadi kesalahan server.' });
  }
...
// POST /api/auth/verify-pin-otp
router.post('/verify-pin-otp', async (req, res) => {
  const { temp_token, otp_code } = req.body;
  if (!temp_token || !otp_code) return res.status(400).json({ error: 'Missing token or OTP' });
  
  const decoded = verifyTempToken(temp_token, 'reset_pin_otp');
  if (!decoded) return res.status(401).json({ error: 'Sesi kedaluwarsa atau tidak valid.' });
  ...
  try {
    ...
    return res.json({ temp_token: signTempToken(decoded.id, 'set_pin'), message: 'OTP verified' });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: 'Terjadi kesalahan server.' });
  }
